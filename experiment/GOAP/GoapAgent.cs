...
public sealed class GoapAgent : MonoBehaviour,IEventRegister,IDataContainer {

	private GoapFSM stateMachine;

	private GoapFSM.GoapFSMState idleState; // finds something to do
	private GoapFSM.GoapFSMState moveToState; // moves to a target
	private GoapFSM.GoapFSMState performActionState; // performs an action
    ...
    public void OnEventRegist(EventCenter evc)
    {
        center=evc;
        Init();
    }
    ...
    {//只序列化statemachine，availableActions，currentactions
	...
	private void loadActions ()
	{
		...
		{
			//按名字生成类对象add到availableaction
			try
			{
				Type type = Type.GetType(actions[i]); 
				availableActions.Add((GoapAction)type.Assembly.CreateInstance(type.Name));
			}
			catch (System.Exception)
			{
				
				Debug.Log("此类"+actions[i]+"不属于goapaction");
			}
...
/*
https://gitee.com/baiyun_is_lovely/goap/blob/master/Assets/Standard%20Assets/Scripts/AI/GOAP/GoapAgent.cs#
*/